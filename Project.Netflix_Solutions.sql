...
FROM 
		netflix
WHERE 
		(YEAR(NOW())-SUBSTRING(date_added,14)) <= 5;
...
WITH genres AS (
			...
			WHERE 
				numbers.n <= LENGTH(listed_in) - LENGTH(REPLACE(listed_in,',',''))+1
	)
SELECT 
		genre,
...
SELECT 
show_type,
TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(casts,',',numbers.n),',',-1)) actors,
country,
COUNT(*) num_of_contents
FROM netflix
CROSS JOIN  (     
				...
				SELECT 46 UNION ALL SELECT 47 UNION ALL SELECT 48 UNION ALL SELECT 49 UNION ALL SELECT 50) numbers
WHERE 
LENGTH(casts)-LENGTH(REPLACE(casts,',',''))+1
GROUP BY SUBSTRING_INDEX(SUBSTRING_INDEX(casts,',',numbers.n),',',-1) 
